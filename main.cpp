...
// if(v != nullptr)
int64_t benchmarkPr(std::vector<std::pair<size_t, std::string>> *v = NULL,
                    size_t req = 1, int reqSize = 32) {
  esdm_rpcc_init_unpriv_service(NULL);
  std::cout << "t\n";
...
void benchmarkMeassureEntropy(meOut &v, long time = -1, long deltaMs = -1) {
  ...
  while (!fullySeeded || (dur < maxDur)) {
    d1 = Clock::now();
    if (std::chrono::duration_cast<ns>(d1 - now) < delta && !firstLoop &&
        useDelta) {
      continue;
    }
    now = Clock::now();
...
int start_pr(const std::vector<std::string> &v, std::string outname,
             bool saveRandomOutput = false) {
  ...
  std::vector<std::pair<size_t, std::string>> *retVecP =
...
int start_me(const std::vector<std::string> &v, std::string outname) {
  ...
  if (v.size() > 3) {
    std::cout
        << "to many arguments for the me benchmark\n. Expected at most 3 "
           "Arguments:\n"
        << "(0) type: 'me'\n"
...
int call_benchmark(const std::vector<std::string> &v,
  ...
  if (v.size() == 0) {
    std::cout
        ...
        << "\t time:\tFor how long the entropy status is meassured (default: "
           "until fully seeded)\n"
        << "\t deltaMs:\tOnly read the entropy status after 'deltaMs' "
           "milliseconds (default: 0) have passed since the last time\n"
  ...
  for (int i = 0; i < repetitions; i++) {
    ...
    std::string outname =
        outdir + tStr + "." + iStr + "." + vm["of"].as<std::string>();
    std::cout << outname << "\n";
    switch (t) {
    case pr:
  ...
  try {
    ...
    desc.add_options()("help,h", "Help screen")(
        ...
        value<std::vector<std::string>>()
            ...
            ->zero_tokens()
            ->composing(),
        "Benchmark")("r,repetitions", value<int>()->default_value(1),
                     "Repetitions of choosen Programm") //<--todo
        ("rng", value<int>()->default_value(32),
         "call esdm_rpcc_get_random_bytes_pr with specified request size")(
            "t,test", value<std::string>(), "call test functions")(
            "of", value<std::string>()->default_value("data.bench"),
            "Suffix of output file. Name of output: "
            "'benchmarkType'.'repetition'.'suffix'")(
            "od", value<std::string>()->default_value("./res/"),
            "Result directory")("save", value<bool>()->default_value(false),
                                "Do save returned values from esdm_rpcc "
                                "functions called in benchmarks: pr");
